...
/**
 * New Donation Email (Team Fundraiser)
 *
 * @since 1.5.0
 */
class DonationTeamFundraiser extends Give_Email_Notification
{
    public $payment;
    ...
    public function init()
    {
        ...
        $this->load(
            [
                'id' => 'p2p-new-donation-team-fundraiser',
                'label' => __('New Donation Team Fundraiser', 'give-peer-to-peer'),
                'description' => __(
                ...
                'recipient_group_name' => __('Team Fundraiser', 'give-peer-to-peer'),
                'notification_status' => 'enabled',
                'email_tag_context' => ['donor', 'donation', 'general', 'p2p', 'team', 'fundraiser'],
        ...
        if ('disabled' != $this->get_notification_status()) {
            add_action("give_new-donation_email_notification", [$this, 'sendEmailNotificationToTeamFundraiser']);
        }
    }
    ...
    public function changeRecipientSettingField(array $recipient, Give_Email_Notification $email): array
    {
        if ($this->config['id'] === $email->config['id']) {
            ...
            $recipient['desc'] = __(
                'This email is automatically sent to the team fundraiser and the recipient cannot be customized.',
                'give-peer-to-peer'
            );
        }
     ...
     * @param int $paymentId
     */
    public function sendEmailNotificationToTeamFundraiser(int $paymentId)
    {
