...
/**
 * New Admin Fundraiser Profile Created Email
 *
 * @since 1.5.0
 */
class AdminFundraiserJoined extends Give_Email_Notification
{
    ...
    public function init()
    {
        $this->load(
            [
                'id' => 'p2p-admin-new-fundraiser-joined',
                'label' => __('New Fundraiser Joined - No Approval Needed', 'give-peer-to-peer'),
                'description' => __('Sent to administrator when a new fundraiser has joined a campaign.',
                    'give-peer-to-peer'),
                'has_recipient_field' => true,
                'recipient_group_name' => $this->get_recipient(),
                'notification_status' => 'enabled',
                'has_preview_header' => true,
                'email_tag_context' => ['donor', 'donation', 'general', 'p2p', 'admin', 'fundraiser'],
                'form_metabox_setting' => false,
                'default_email_subject' => esc_attr__('New Fundraiser Created - No Approval Needed!',
                    'give-peer-to-peer'),
                'default_email_message' => $this->getDefaultEmailMessage(),
                'default_email_header' => __('New Fundraiser Created - No Approval Needed', 'give-peer-to-peer'),
            ]
        );
        ...
        if ('disabled' != $this->get_notification_status()) {
            add_action('give_p2p_fundraiser_created', [$this, 'sendEmailNotificationToAdmin']);
        }
    }
    ...
    public function getDefaultEmailMessage(): string
    {
        $defaultEmailMessage = sprintf(
            ...
            esc_html__('A new fundraiser has joined %s!', 'give-peer-to-peer') . "\n\n" .
            esc_html__('Fundraiser Name: ', 'give-peer-to-peer') . '%s' . "\n\n" .
            esc_html__('View all fundraisers here: ',
                'give-peer-to-peer') . '<a href="%s" target="_blank" rel="noreferrer">%s</a>' . "\n\n"
            , '{admin_email}', '{campaign_name}', '{fundraiser_first_name}', '{view_all_fundraisers_url}',
    ...
    public function emailPreviewHeader(string $emailPreviewHeader, Give_Email_Notification $email): string
    {
        ...
        // Default option.
        $options[0] = esc_html__('No fundraisers found.', 'give-peer-to-peer');
        ...
        <div class="give_preview_email_user_id_main">
            ...
            <label for="give_preview_email_user_id" class="give_preview_email_user_id_label">
                <?php
                echo esc_html__('Select a fundraiser to populate the preview with real information:',
                    'give-peer-to-peer'); ?>
            </label>
     ...
     *
     */
    public function sendEmailNotificationToAdmin(Fundraiser $fundraiser)
    {
