...
/**
 * New Fundraiser Profile Created Email
 *
 * @since 1.5.0
 */
class FundraiserProfileCreated extends Give_Email_Notification
{
    ...
    public function init()
    {
        $this->load(
            [
                'id' => 'p2p-new-fundraiser-profile-created',
                'label' => __('New Fundraiser Profile Created', 'give-peer-to-peer'),
                'description' => __('Sent to designated recipient(s) when a new fundraiser profile is created.',
                    'give-peer-to-peer'),
                ...
                'default_email_subject' => esc_attr__('Your fundraiser profile has been created!',
                    'give-peer-to-peer'),
                'default_email_message' => $this->getDefaultEmailMessage(),
                'default_email_header' => __('New Fundraiser Created', 'give-peer-to-peer'),
            ]
        );
        ...
        if ('disabled' != $this->get_notification_status()) {
            add_action('give_p2p_fundraiser_created', [$this, 'sendEmailNotificationToFundraiser']);
        }
    }
    ...
    public function getDefaultEmailMessage(): string
    {
        $defaultEmailMessage = sprintf(
            ...
            esc_html__('Your fundraising profile for %s has been created!', 'give-peer-to-peer') . "\n\n" .
